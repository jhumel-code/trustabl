...
Every policy YAML file in `internal/rules/policies/` requires a paired rationale document at:
...
| YAML file | Doc file |
|---|---|
| `internal/rules/policies/claude_sdk/network.yaml` | `docs/Policy/claude_sdk/network.md` |
| `internal/rules/policies/openshell/shell.yaml` | `docs/Policy/openshell/shell.md` |
| `internal/rules/policies/mcp/injection.yaml` | `docs/Policy/mcp/injection.md` |
| `internal/rules/policies/catalog/capability_class.yaml` | `docs/Policy/catalog/capability_class.md` |

Category subdirectories under `docs/Policy/` mirror `internal/rules/policies/` exactly.
If a new category directory is created in policies, create the matching directory in `docs/Policy/`.

---

## Cross-referencing related docs

When a rule duplicates or reinforces a rule from another category (e.g., CATL-003
and CSDK-004 both cover path traversal), the lower-priority doc should cross-reference
the primary doc rather than repeat the full rationale:
...
| Singleton / manifest rule | [Policy/openshell/resources.md](Policy/openshell/resources.md) |
